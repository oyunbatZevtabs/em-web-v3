...
const UglugKhaakhModal = ({ turul, token, mutate }) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setKhevlekhData({
        ...khevlekhData,
        zarlaga: songogdsonNer?.uglugUldegdel,
      });

      form.setFieldValue("zarlaga", songogdsonNer?.uglugUldegdel);
    }
  }, [songogdsonNer]);
  ...
  function uglugKhadgalyaa(formdata) {
    if (!songogdsonNer?.uglugUldegdel) {
      return AnkhaaruulgaAlert("Өглөг үүсээгүй байна");
    }
  ...
  return (
    <div>
      ...
      <Modal
        ...
        title={<div className="font- text-center text-[20px] ">Өглөг хаах</div>}
        open={modalOpen}
      ...
      >
        <div>
          ...
          >
            ...
            {!!songogdsonNer && (
              <div className="-mt-2 flex gap-3 border-t py-2 pl-16 font-medium">
                Өглөгийн үлдэгдэл:{" "}
                <div>{formatNumber(songogdsonNer?.uglugUldegdel, 2) || 0}₮</div>
              </div>
            )}
            ...
            >
              <InputNumber
                ...
                max={songogdsonNer?.uglugUldegdel}
...
export default UglugKhaakhModal;
