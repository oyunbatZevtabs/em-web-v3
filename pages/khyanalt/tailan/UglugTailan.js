...
function UglugTailan({ token, khariltsagchiinId }) {
  ...
  const { tailan, unshijBaina } = useUglugTailan(
    token,
    shuult,
    undefined,
    searchKeys,
  ...
  async function exceleerTatya() {
    ...
    await posUilchilgee(token)
      .post("/uglugTovchooTailanAvya", {
        ...shuult,
        khuudasniiKhemjee: tailan?.khuudasniiKhemjee,
      })
      .then(({ data }) => {
        excel
          .addSheet("Өглөгийн товчоо тайлан")
          .setTHeadStyle({
            background: "ffccf8f8",
          ...
          .addDataSource(data)
          .saveAs("Өглөгийн товчоо тайлан.xlsx");
        setLoading(false);
      })
      .catch((err) => {
        // aldaaBarigch(err);
  ...
  const columns = useMemo(() => {
    ...
    return jagsaalt;
  }, [shineBagana, salbaruud, tailan, ognoo, salbariinId]);
      ...
      <div className="col-span-12 flex flex-col gap-6 px-5 md:px-3">
        <div className="flex">
          <div className="flex w-full gap-2">
            ...
            <Select
              bordered={false}
              className="!h-[40px] w-40  rounded-md border-[1px] border-[#4FD1C5] bg-white dark:bg-gray-800"
              showSearch
              mode="multiple"
              filterOption={(o) => o}
              allowClear={true}
