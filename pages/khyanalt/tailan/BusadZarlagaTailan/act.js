          ...
          >
            {mur.summary ? (
              mur.title === "№" ? (
                ...
                formatNumber(tooKhemjeeSum || 0, 2) + "ш"
              ) : mur.title === "Нийт" ? (
...
function Act({ token, setOgnoo, ognoo }) {
  ...
  const actShuult = useMemo(() => {
    ...
    return query;
  }, [ognoo, baiguullagiinId, salbariinId, filterSalbariinId, baraanuud]);

  const { tailanGaralt, unshijBaina } = useActTailan(
    token,
    actShuult,
    undefined,
    order
        ...
        excel
          ...
          .addColumns([
            ...
            },
            {
              ...
              __style__: { h: "right" },
              render: (tooKhemjee) => {
                return formatNumber(tooKhemjee);
              },
            },
  ...
  const columns = useMemo(() => {
    ...
    return jagsaalt;
  }, [shineBagana, salbaruud, tailanGaralt, ognoo]);
        ...
        <div className="mb-5 flex w-full   gap-2 md:w-[50%] md:flex-row">
          ...
          <div className="flex w-[50%] gap-2">
            ...
            >
              {salbaruud?.map((v) => (
                <Select.Option key={v?._id} value={v?._id}>
                  <div className="flex flex-row overflow-hidden ">{v?.ner}</div>
                </Select.Option>
              ))}
            </Select>
            ...
            <Select
              ...
              allowClear
              mode="multiple"
              className="ц !h-[36px] w-[50%] rounded-md  border-[1px] border-[#4fd1c5] bg-white md:w-40"
              filterOption={(a) => a}
              onChange={(v) => {
                setSongosonBaraa(v);
                v === undefined &&
